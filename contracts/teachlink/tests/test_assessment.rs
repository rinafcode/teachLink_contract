...
fn test_create_assessment() {
    ...
    let assessment_id =
        client.create_assessment(&creator, &title, &description, &questions, &settings);
...
fn test_adaptive_selection() {
    ...
    let assessment_id = client.create_assessment(
        &creator,
        &Bytes::from_slice(&env, b"Adaptive Quiz"),
        &Bytes::from_slice(&env, b"Desc"),
        &questions,
        &AssessmentSettings {
            time_limit: 0,
    ...
    let next_q = client.get_next_adaptive_question(&assessment_id, &scores, &answered);

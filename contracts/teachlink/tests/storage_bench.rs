...
use soroban_sdk::{testutils::Address as _, Address, Env, Map, symbol_short};
use crate::{TeachLinkBridge, types::{LPPosition, LiquidityPool}};
...
fn test_storage_efficiency() {
    ...
    for _ in 0..50 {
         large_map.set(Address::generate(&env), LPPosition {
    ...
    env.storage().instance().set(&symbol_short!("L_MAP"), &large_map);
    env.budget().reset_unlimited();
    let mut loaded: Map<Address, LPPosition> = env.storage().instance().get(&symbol_short!("L_MAP")).unwrap();
    loaded.set(provider.clone(), LPPosition {
    ...
    env.storage().instance().set(&symbol_short!("L_MAP"), &loaded);
    let update_gas_old = env.budget().cpu_instruction_count();
    
    println!("Optimized update gas: {}", update_gas_optimized);
    println!("Old approach simulated gas (50 entries): {}", update_gas_old);
    
    assert!(update_gas_optimized < update_gas_old, "Optimized approach should be much cheaper");
}

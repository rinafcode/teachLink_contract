...
fn test_reputation_system_integration() {
    ...
    bridge_client.leave_review(
        &user2,
        &content_id,
        &5,
        &Bytes::from_slice(&env, b"Excellent course!"),
    );
...
fn test_cross_chain_atomic_swap() {
    ...
    let swap_id = bridge_client.initiate_atomic_swap(
        &token1, &1000, &user2, &hashlock, &timelock, &1, // chain 1
    );
    ...
    let reverse_swap_id = bridge_client.initiate_atomic_swap(
        &token2, &1500, &user1, &hashlock, &timelock, &2, // chain 2
    );

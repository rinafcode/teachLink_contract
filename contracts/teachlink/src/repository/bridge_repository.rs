...
impl<'a> BridgeConfigRepository<'a> {
    ...
    pub fn get_fee_recipient(&self) -> Result<Address, StorageError> {
        self.storage.get(&FEE_RECIPIENT).ok_or(StorageError::NotFound)
    }
...
impl<'a> ValidatorRepository<'a> {
    ...
    pub fn get_validators(&self) -> Map<Address, bool> {
        self.storage.get(&VALIDATORS).unwrap_or_else(|| Map::new(self.storage.env()))
    }
...
impl<'a> ChainRepository<'a> {
    ...
    pub fn get_supported_chains(&self) -> Map<u32, bool> {
        self.storage.get(&SUPPORTED_CHAINS).unwrap_or_else(|| Map::new(self.storage.env()))
    }
...
impl<'a> BridgeTransactionRepository<'a> {
    ...
    pub fn get_transactions(&self) -> Map<u64, BridgeTransaction> {
        self.storage.get(&BRIDGE_TXS).unwrap_or_else(|| Map::new(self.storage.env()))
    }
...
impl<'a> BridgeRetryRepository<'a> {
    ...
    pub fn get_retry_counts(&self) -> Map<u64, u32> {
        self.storage.get(&BRIDGE_RETRY_COUNTS).unwrap_or_else(|| Map::new(self.storage.env()))
    }
    ...
    pub fn get_last_retry(&self) -> Map<u64, u64> {
        self.storage.get(&BRIDGE_LAST_RETRY).unwrap_or_else(|| Map::new(self.storage.env()))
    }
    ...
    pub fn get_failures(&self) -> Map<u64, Bytes> {
        self.storage.get(&BRIDGE_FAILURES).unwrap_or_else(|| Map::new(self.storage.env()))
    }

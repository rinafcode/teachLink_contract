...
use crate::reputation::{rate_contribution, update_course_progress, update_participation};
...
fn test_bridge_cancel_emits_event() {
    ...
    env.as_contract(&contract_id, || {
        ...
        env.storage()
            .instance()
            .set(&crate::storage::BRIDGE_TXS, &txs);

        // Move time forward to allow cancellation
        env.ledger()
            .with_mut(|li| li.timestamp = timestamp + 700_000);
...
fn test_validator_management_emits_events() {
    ...
    env.as_contract(&contract_id, || {
        ...
        env.storage()
            .instance()
            .set(&crate::storage::VALIDATORS, &validators);
...
fn test_circuit_breaker_reset_emits_event() {
    ...
    env.as_contract(&contract_id, || {
        ...
        EmergencyManager::check_circuit_breaker(&env, 1, 200000).expect_err("should trigger");
...
fn test_insurance_pool_emits_events() {
    ...
    env.as_contract(&contract_id, || {
        ...
        // Initialize pool
        InsuranceManager::initialize_pool(&env, token.clone(), 100).expect("init should succeed");
        ...
        // Fund pool
        InsuranceManager::fund_pool(&env, funder.clone(), 10000).expect("fund should succeed");
...
fn test_escrow_cancel_emits_event() {
    ...
    env.as_contract(&contract_id, || {
        ...
        // Cancel escrow
        EscrowManager::cancel(&env, escrow_id, depositor.clone()).expect("cancel should succeed");
...
fn test_bridge_fee_update_emits_event() {
    ...
    env.as_contract(&contract_id, || {
        ...
        env.storage()
            .instance()
            .set(&crate::storage::FEE_RECIPIENT, &fee_recipient);
        env.storage()
            .instance()
            .set(&crate::storage::BRIDGE_FEE, &0i128);
        env.storage()
            .instance()
            .set(&crate::storage::MIN_VALIDATORS, &1u32);

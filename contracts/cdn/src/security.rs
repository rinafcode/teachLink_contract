...
impl SecurityManager {
    ...
    pub fn enable_drm(
        ...
        let mut drm_configs: Map<String, DRMConfig> = env.storage().instance()
            .get(&DRM_CONFIGS)
            .unwrap_or_else(|| Map::new(env));
    ...
    pub fn generate_access_token(
        ...
        let drm_configs: Map<String, DRMConfig> = env.storage().instance()
            .get(&DRM_CONFIGS)
            .unwrap_or_else(|| Map::new(env));

        let drm_config = drm_configs.get(content_id.clone())
        ...
        let access_tokens: Map<String, AccessToken> = env.storage().instance()
            .get(&ACCESS_TOKENS)
            .unwrap_or_else(|| Map::new(env));
        ...
        let token_counter: u64 = env.storage().instance().get(&symbol_short!("TOK_CNT")).unwrap_or(0);
        let new_counter = token_counter + 1;
        env.storage().instance().set(&symbol_short!("TOK_CNT"), &new_counter);
        ...
        env.storage().instance().set(&ACCESS_TOKENS, &updated_tokens);
    ...
    pub fn revoke_access_token(
        ...
        let mut access_tokens: Map<String, AccessToken> = env.storage().instance()
            .get(&ACCESS_TOKENS)
            .unwrap_or_else(|| Map::new(env));

        let mut access_token = access_tokens.get(token.clone())
            .ok_or(CDNError::InvalidToken)?;
    ...
    pub fn get_drm_config(
        env: &Env,
        content_id: String,
    ) -> Result<Option<DRMConfig>, CDNError> {
        let drm_configs: Map<String, DRMConfig> = env.storage().instance()
            .get(&DRM_CONFIGS)
            .unwrap_or_else(|| Map::new(env));
